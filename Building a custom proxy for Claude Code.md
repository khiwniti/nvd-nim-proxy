Building a custom proxy for Claude Code primarily involves pointing it to your own server via `ANTHROPIC_BASE_URL`. Here's what you need to know:

## How Claude Code Connects to the API

Claude Code uses the Anthropic SDK under the hood and sends requests to `api.anthropic.com` by default. You redirect this with:

```bash
export ANTHROPIC_BASE_URL=https://your-proxy.example.com
export ANTHROPIC_API_KEY=your-key-or-passthrough-key
```

## What Your Proxy Must Implement

Your proxy must be **compatible with the Anthropic Messages API**. Specifically:

1. **Streaming SSE endpoint** — Claude Code uses streaming by default. Your proxy must correctly forward `text/event-stream` responses. A known issue was that 404 errors on streaming endpoints triggered a non-streaming fallback, which was fixed in v2.1.33. [1](#0-0) 

2. **Beta headers** — Claude Code sends experimental beta headers. If your proxy/gateway rejects unknown headers, set `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1`. This was specifically fixed in v2.1.77 where beta tool-schema fields were not being stripped, causing proxy gateways to reject requests. [2](#0-1) 

3. **Tool use blocks** — Claude Code uses `tool_use` and `tool_result` message blocks extensively. When using `ANTHROPIC_BASE_URL` with a third-party gateway, Claude Code now auto-detects proxy endpoints and disables `tool_reference` blocks that some gateways reject (fixed in v2.1.70). [3](#0-2) 

## Network Proxy (HTTP/HTTPS Tunnel)

If you need a network-level proxy (not an API-level one):
...
- `NO_PROXY` support was added in v1.0.93. [4](#0-3) 
- `HTTP_PROXY`/`HTTPS_PROXY` for OpenTelemetry was added in v2.0.17. [5](#0-4) 
- Proxy DNS resolution is **opt-in** via `CLAUDE_CODE_PROXY_RESOLVES_HOSTS=true` (it was forced on by default and fixed in v2.0.55). [6](#0-5) 

## Sandbox + MITM Proxy

If you're running in sandbox mode and need a MITM proxy (e.g., for TLS inspection), use the `httpProxyPort` sandbox setting combined with `sandbox.enableWeakerNetworkIsolation` (macOS only) to allow Go-based tools like `gh`, `gcloud`, and `terraform` to verify TLS certificates against your custom CA. [7](#0-6) 

## Known Compatible Proxy Solutions

The CHANGELOG references **LiteLLM** as a known-compatible proxy (loopback config was updated in v1.0.32). [8](#0-7) 

## Key Gotchas

| Issue | Fix |
|---|---|
| Proxy settings not applied to `WebFetch` | Fixed in v2.1.33; ensure proxy env vars are set before launch |
| mTLS / client certificates | Fixed in v2.1.23 |
| Beta headers rejected by gateway | Set `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` |
| `tool_reference` blocks rejected | Use `ANTHROPIC_BASE_URL`; Claude Code auto-detects and disables them | [9](#0-8) [10](#0-9) 

In summary: the cleanest approach is to implement an **Anthropic-compatible Messages API** server (LiteLLM, a custom Node/Python server, etc.) and point Claude Code at it via `ANTHROPIC_BASE_URL`. The official docs at `https://code.claude.com/docs/en/overview` may have more up-to-date gateway/proxy guidance. [11](#0-10)

### Citations

**File:** CHANGELOG.md (L904-904)
```markdown
...
**File:** CHANGELOG.md (L2439-2439)
```markdown
- OTEL: support HTTP_PROXY and HTTPS_PROXY
```

**File:** CHANGELOG.md (L2629-2629)
```markdown
- Support NO_PROXY environment variable to bypass proxy for specified hostnames and IPs
```

**File:** CHANGELOG.md (L2908-2908)
```markdown
...
```
Here's how to wire NVIDIA NIM (open-source models) into Claude Code via a LiteLLM translation proxy:

## Why a Translation Layer is Needed

Claude Code speaks the **Anthropic Messages API** format. NVIDIA NIM exposes an **OpenAI-compatible API**. You need a proxy that translates between the two. LiteLLM is the documented-compatible solution for this. [1](#1-0) 
...
## Step 1: LiteLLM Proxy Config

Create `litellm_config.yaml`:

```yaml
model_list:
  # Map the model name Claude Code will request → NVIDIA NIM model
  ...
  # Optionally map the "small/fast" model too
...
general_settings:
  # LiteLLM will expose an Anthropic-compatible endpoint
...
## Step 2: Point Claude Code at Your Proxy

```bash
export ANTHROPIC_BASE_URL=http://localhost:4000
export ANTHROPIC_API_KEY=anything        # LiteLLM ignores this; uses its own key
export CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1   # strip beta headers gateways reject
```

The `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` flag is critical — without it, Claude Code sends beta tool-schema fields that third-party gateways reject with 400 errors. [2](#1-1) 

Claude Code also auto-detects when `ANTHROPIC_BASE_URL` is set and disables `tool_reference` blocks that non-Anthropic gateways reject. [3](#1-2) 

---

## Step 3: Customize the Model Picker (Optional)

To make the `/model` picker show your NVIDIA-backed model:

```bash
export ANTHROPIC_CUSTOM_MODEL_OPTION=meta/llama-3.1-70b-instruct
export ANTHROPIC_CUSTOM_MODEL_OPTION_NAME="Llama 3.1 70B (NVIDIA NIM)"
export ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION="Open-source via NVIDIA NIM"
``` [4](#1-3) 

You can also override the default model labels shown in the picker:

```bash
export ANTHROPIC_DEFAULT_SONNET_MODEL_NAME=meta/llama-3.1-70b-instruct
export ANTHROPIC_DEFAULT_HAIKU_MODEL_NAME=meta/llama-3.1-8b-instruct
``` [5](#1-4) 
...
| Issue | Fix |
|---|---|
| Gateway rejects beta headers | `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1` |
| `tool_reference` blocks rejected | Auto-handled when `ANTHROPIC_BASE_URL` is set |
| 404 on streaming endpoint | Fixed in v2.1.33; ensure LiteLLM SSE is working |
| Proxy settings not applied to WebFetch | Fixed in v2.1.33; set env vars before launch |
| `effort` parameter rejected by model | Fixed in v2.1.70 for non-standard model IDs | [6](#1-5) [7](#1-6) 

---

## Important Limitation

Claude Code is purpose-built for Claude models. Open-source models via NVIDIA NIM will lack Claude-specific capabilities like extended thinking, interleaved thinking, and some tool-use patterns. The quality of agentic coding tasks will vary significantly depending on the model chosen.

### Citations

**File:** CHANGELOG.md (L673-673)
```markdown
...
## Max Token Error in NVIDIA Proxy - Missing Thinking Parameter Handling
This codemap identifies the root cause of the 'max_tokens must be greater than thinking.budget_tokens' error in the NVIDIA proxy. The proxy translates Anthropic requests to OpenAI format but drops the thinking parameter without adjusting max_tokens accordingly [1a][1b]. Claude Code previously encountered this same issue and fixed it [1d]. The solution requires adding thinking parameter handling to extract budget_tokens and ensure max_tokens exceeds it [1e].
### 1. Request Translation Flow - Missing Thinking Parameter
Traces how the proxy translates Anthropic requests to OpenAI format, highlighting where the thinking parameter is dropped and max_tokens is set without accounting for thinking budget
### 1a. Default max_tokens set to 4096 (`proxy.py:213`)
The proxy sets a default max_tokens of 4096 without checking if thinking is enabled in the request
```python
payload: dict = {
    "model": body["model"],
    "messages": msgs,
...
```
### 1b. Standard parameters forwarded (`proxy.py:219`)
Standard Anthropic parameters are forwarded, but the thinking parameter is not included in this list
```python
for key in ("temperature", "top_p", "top_k"):
    if (v := body.get(key)) is not None:
        payload[key] = v
```
### 1c. Thinking blocks dropped in messages (`proxy.py:227`)
The proxy explicitly drops thinking blocks from messages, but doesn't handle the top-level thinking parameter
```python
...
```
### 1d. Claude Code bug fix for max_tokens validation (`CHANGELOG.md:2370`)
Claude Code fixed an issue where the API rejected requests when max_tokens wasn't greater than thinking.budget_tokens
```text
- Fixed a bug where `/context` would sometimes fail with "max_tokens must be greater than thinking.budget_tokens" error message
```
### 1e. Required fix: Add thinking parameter handling (`proxy.py:219`)
The proxy needs to extract the thinking.budget_tokens and ensure max_tokens is set higher than the budget
```python
for key in ("temperature", "top_p", "top_k"):
    if (v := body.get(key)) is not None:
        payload[key] = v
# ADD HERE: Handle thinking parameter
if (thinking := body.get("thinking")):
    budget_tokens = thinking.get("budget_tokens", 0)
    max_tokens = body.get("max_tokens", 4096)
    if max_tokens <= budget_tokens:
        payload["max_tokens"] = budget_tokens + 4096  # Ensure max_tokens > budget_tokens
```
