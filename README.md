...
NVIDIA_API_KEY=$(grep ^NVIDIA_API_KEY .env | cut -d= -f2) python proxy.py
...
> The four `*_MODEL` aliases are required. Without them, Claude Code's
> built-in fallbacks resolve to Anthropic model names like
> `claude-haiku-4-5-20251001` and NVIDIA returns 404.
...
```
proxy.py              # the entire proxy (~600 lines)
